...
final class ValuesTest extends TestCase
{
    ...
    /** @test IsDate() deprecated - verify legacy behavior and that deprecation is triggered */
    public function testIsDate(): void
    {
        // Custom error handler to capture ONLY the expected IsDate deprecation
        // This keeps test output clean while verifying the deprecation fires
        $deprecationTriggered = false;
        $prevHandler = set_error_handler(function ($severity, $message) use (&$deprecationTriggered) {
            if ($severity === E_USER_DEPRECATED && str_contains($message, 'IsDate() deprecated')) {
                $deprecationTriggered = true;
                return true; // Silence this specific expected deprecation
            }
            return false; // Let all other errors/deprecations propagate normally
        });
        ...
        try {
            ...
            // Verify the deprecation was actually triggered (at least once)
            $this->assertTrue($deprecationTriggered, 'IsDate() should trigger E_USER_DEPRECATED');
        } finally {
